...
def prepare_multi_scale_data(src_path, dst_path, gap=200, subsize=1024, scales=[0.5, 1.0, 1.5], num_process=32):
    ...
    split_train = splitbase_trainval(osp.join(src_path, 'train_val'), dst_trainval_path,
                                     gap=gap, subsize=subsize, num_process=num_process)
    for scale in scales:
        split_train.splitdata(scale)
    print('split val data')
    # split val data
    split_val = splitbase_trainval(osp.join(src_path, 'test'), dst_trainval_path,
                                   gap=gap, subsize=subsize, num_process=num_process)
    for scale in scales:
...
# FAIR1M_plane_1_1024_512
if __name__ == '__main__':
    prepare_multi_scale_data('../../FAIR1M_plane_1_1024_512', '../data/fair1m', gap=200, subsize=1024, scales=[1.0],
                             num_process=1)
