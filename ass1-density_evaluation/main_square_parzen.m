...
h = 0.5:0.1:5;  % h代表窗口的长度
global h_glb;
h_glb = h(1);

% 用不同的窗口长度来预测概率，并且对其评估，找到最优的窗口长度
mse = zeros(size(h,2),1);
for i=1:size(h,2)
    h_glb = h(i);
    result = cellfun(@square_parzen_predict,num2cell(X_test,2));  % num2cell负责把数据切开，dim=2就可以一行一行的输入
    mse(i) = sum((P_test-result).^2)/test_size;
end
h_best = h(find(mse==min(mse)));  % find负责返回满足条件的索引
h_best = h_best(1);  % 有可能存在多个，只要第一个
h_glb = h_best;

result = cellfun(@square_parzen_predict,num2cell(X_test,2));
...
title('parzen窗口法（方窗）估计概率密度');

% 有真实值和预测值两组3维数据，所以要分别指定属性
% 分别指定颜色
color1 = [0.1 0.1 0.1];  % 黑色
color2 = [0.9 0 0];  % 红色
% 散点大小
...
f = getframe(gcf);
path = ['res','/square_parzen'];
file_name = ['/square_parzen_',num2str(train_size),'_',num2str(test_size),'.png'];
if ~exist(path,'dir')
    mkdir(path);
end
imwrite(f.cdata,[path, file_name]);
file_name = ['/square_parzen_',num2str(train_size),'_',num2str(test_size),'.txt'];
